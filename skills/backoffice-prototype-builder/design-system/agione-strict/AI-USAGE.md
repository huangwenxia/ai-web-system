...
- 每页都默认站在“当前目标用户第一次进入页面”的视角判断是否成立。先判断用户角色和页面任务，再决定信息展示深度；目标用户可能是业务用户、开发者、运维、管理员或集成用户。
- 首屏必须快速说明对象/范围、关键状态/结果/上下文、下一步关注点/动作和主操作结果。若页面需要解释才能看懂，先收敛主任务和业务表达，不继续堆模块、说明、卡片、标签、指标或装饰。
- 列表页强调范围、筛选、记录状态和行操作；详情页强调对象身份、关键状态、主要配置和关联入口；表单页强调填写目标、必填重点和提交后果；流程页强调当前步骤、阻塞原因和下一步动作；Dashboard 强调整体状态、异常和下钻路径；API/开发者页强调接口状态、调用方式、必要参数、复制入口、错误处理和调试路径。
- 客户可见文案不得出现与当前任务无关的内部技术证据，例如 `Api.general.xxx`、`result.total`、`hasXxx`、`currentStep`、`AI-NOTES`、`data-source`、`mock`、前端路由、源码 API client 名称、代码变量、状态判断表达式、“数据来源 / 状态判断来源 / 根据规则推导”等。
- 如果页面本身是 API / SDK / 调试 / 日志 / 事件追踪，可以展示接口 URL、请求方法、Headers、Body、示例代码、响应示例、错误码和复制入口；但不得展示前端源码 client 名称、原型数据来源说明或 AI 设计备注，敏感信息必须脱敏，低频诊断信息应折叠。
- 输出前做目标用户 5 秒自检：5 秒内能否理解页面目的、当前状态、下一步关注点/动作和动作结果；页面是否已清理无关代码感、接口感、内部规则感或原型说明感内容。
