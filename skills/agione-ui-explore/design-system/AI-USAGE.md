...
## 主业务锚定 + 语义 / Attention 清洗（v2.17 · 总纲 · explore 同守）

> 生成时的 gate,不是事后清洗——加每处文字/图标/强调前先过。explore 多 variant 时**每个 variant 各自过一遍**(构图可不同,但"围绕主业务、不堆装饰"是 base spec 级,不放飞)。完整版见 strict AI-USAGE §总纲。

- **先锚主业务目标**(1 主 +至多 1 次;REQ明文 > 页型默认 > 拿不准问)。
- **二级漏斗**:gate A 存在性 "删了它用户完成主业务丢什么?" 答"没什么"→删;gate B 强调性 "第一眼吸引但不导向理解/行动?"→弱化。
- **图标语义**(硬规则):图标只留承担 affordance/状态语义的(可点/可复制/可跳转/可展开/不可用/错误/状态);❌ 不装饰、不填空、不误导可点;**affordance 图标(copy/external/chevron)≠ 重复,保留**——"重复"只指纯标签复述文字。
- **attention 是一份预算**:色/边/影/字号/图标/插画/动效同预算,一屏主 attention ≤ 1;explore 鼓励**视觉创新**但不是"多个中心互相抢"。
- 无 audit(判断层),靠生成自律 + 自检人眼过。
