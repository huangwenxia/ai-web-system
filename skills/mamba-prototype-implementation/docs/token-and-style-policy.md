...
本文件承接 `mamba-prototype-implementation` 中样式、token、Tailwind、Element Plus 与原型适配的细则。只在任务涉及可见 UI、样式边界、主题兼容或组件封装时读取。

外层组件需要影响嵌套子组件、第三方组件内部结构或 Teleport / Popper 浮层样式时，先读取 `skills/frontend-implementer-skill/docs/style-override-discipline.md`，完成 CSS 作用链定位后再修改。
...
## 原型视觉与项目标准边界
- 已确认原型是视觉与交互输入；project-mamba 当前 token、主题和组件体系是实现边界。
- 只在当前原型落码范围和直接依赖组件内做样式适配，不做全项目视觉治理。
