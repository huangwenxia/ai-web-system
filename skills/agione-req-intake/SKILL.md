...
8. **口径替换，不做叠加覆盖**：Owner 改变已有业务决定时，必须识别并替换当前有效文档中的旧口径；禁止只追加 changelog 或一段新说明，却保留相反的 brief、验收标准或下游文档。
9. **区分扩展与替代**：`Extends` 表示旧规则仍有效且新增能力兼容；`Supersedes` 表示旧规则失效。两者不确定时必须追问，不能自行选择。
10. **逻辑关系不自动变成用户能力**：内部归属、系统自动解析关系、审计或幂等字段等逻辑事实，不能被写成用户可选择、填写或查看的需求，除非用户明确确认存在对应页面任务。
...
| 改动量 | 路径 | 本 Skill 后续动作 |
|-------|------|-----------------|
| 补丁 / 文案 / 小调整 | 不新建 REQ；更新本 Skill 负责的 brief / acceptance-checklist，追加 changelog，并列出所有受影响下游文档 | 跳过新编号生成，但必须执行旧口径检索和一致性收口，不得仅写 changelog 后结束 |
| 新增兼容功能模块 | 新建 REQ 编号，在 brief.md 头部标注 `Extends: REQ-YYYY-NNN` | 走完整 Step 6 流程，并确认父 REQ 规则继续有效 |
| 替代已有核心规则 | 新建 REQ 编号，在 brief.md 头部标注 `Supersedes: REQ-YYYY-NNN`，并在被替代 REQ 的入口或 changelog 标记失效范围 | 走完整 Step 6 流程，输出受影响文档清单；未清理冲突前不得宣告收口 |
...
- 已知约束：{列表}
- 待澄清问题：{列表}
- 与已有口径关系：{全新 / Extends REQ-... / Supersedes REQ-...}
- 需要同步清理的旧口径：{无 / 文档与章节清单}
...
1. 从用户本次确认中提取新口径及被替代的旧口径关键词。
2. 使用 `rg --no-ignore` 检索目标 REQ、关联 REQ、changelog、brief、acceptance、solution、flow、functions 和 prototype 文档。
3. 输出影响清单并分类为：本 Skill 直接修改、需 `/agione-req-refine` 同步、需 `/agione-prototype-prep` 同步、历史材料需标记废弃。
4. 如果同级或高优先级文档仍存在无法判定的冲突，停止生成并请 Owner 决策；不得依靠新 brief 静默覆盖旧规则。
...
- 功能模块分配表必须附在最后
- 当本次决定替代旧规则时，直接改写旧 AC 和业务描述，不同时保留新旧两套口径
...
- 当前有效文档中的相反表述必须清零。
- 逻辑层保留项必须明确标注为内部关系或非页面字段。
- 不在本 Skill 修改权限内的下游文档必须列入阻塞清单，并要求运行对应 Skill；在这些冲突处理完之前，不得描述为“整个 REQ 已完成更新”。
- 完成报告必须列出检索关键词、修改文件、废弃资料和允许保留的残留命中。
