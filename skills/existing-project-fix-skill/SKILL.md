...
- 涉及可见 UI 的修复，默认站在“当前目标用户第一次进入页面”的视角复查：目标用户能否快速理解页面目的、当前状态、下一步关注点/动作和动作结果。页面若需要解释才能看懂，先收敛主任务和业务表达，不靠增加模块、说明、标签或装饰补洞。
- 修复后检查客户可见文案是否存在无关内部技术证据，例如 `Api.general.xxx`、`result.total`、`hasXxx`、`currentStep`、`AI-NOTES`、`data-source`、`mock`、前端路由、源码 API client 名称、状态判断表达式、“数据来源 / 状态判断来源 / 根据规则推导”等；开发者/API/诊断页只展示完成任务所需的技术信息，并对敏感信息脱敏。
...
- 修复完成后做目标用户 5 秒自检，确认用户能理解当前状态、下一步动作和动作结果，且可见 UI 没有无关代码感、接口感、内部规则感或原型说明感内容。
