...
### 6. 组件、状态与副作用修复口径
- 同一份业务状态只保留一个真实来源；能用 `computed` 推导的状态，不额外维护 `ref` 镜像副本。
- 子组件不直接修改 `props`；父组件负责业务编排，子组件负责局部交互和展示。
- 展示组件默认不直接承担接口 orchestration；Store 只在确有跨页面或跨模块共享需求时接入。
- `watch` 只做副作用，不用来维护业务主状态；初始化请求优先 `onMounted` 或显式 `init()`。
- 异步并发要显式处理竞态、最后一次请求生效和清理逻辑，定时器、事件、订阅必须成对清理。

### 7. 模板、样式与可见质量口径
- 模板只负责结构表达，复杂条件应抽成具名 `computed` 或方法；稳定列表禁止长期使用数组索引作为 `key`。
- 可交互区域必须使用语义化可交互元素；Hover、Focus、Pointer 反馈不能错误地出现在纯展示元素上。
- 修复样式问题时，不新增无理由的裸十六进制颜色、魔法间距和魔法高度；如果项目已使用 Tailwind，优先走现有工具类体系。
- 修复后顺手检查同链路的对齐、间距、Key/Value 分离、状态层级和明显 1px 级可见问题，但不扩成大范围重设计。

### 8. 字段、枚举与展示一致性
- 相同业务字段的命名、单位、格式和展示语义必须在列表、详情、编辑和提示文案中保持一致。
- 状态 / 枚举字段优先走集中常量、统一字典组件或统一映射方法，不在局部组件里重复硬编码。
- 所有状态和枚举展示都必须具备未知值兜底，避免因为接口新增值导致空白、报错或语义漂移。
