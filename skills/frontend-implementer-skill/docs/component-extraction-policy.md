...
- 组件抽离不只看是否复用；核心标准是是否形成了独立职责边界。当前只使用一次但已经有清晰业务语义、输入输出、交互状态和样式族的 UI 区块，也应优先纳入抽离候选。
...
## 独立职责边界判断
不要把“只用一次”当成不抽组件的理由。按候选 UI 区块判断，而不是按文件行数或复用次数机械判断。

满足以下任意 3 项时，必须优先考虑抽成页面私有组件；如果继续留在父组件，最终校验中必须说明保留原因：
- 有清晰业务语义和可命名的组件名，例如 `DeployPointSegmentsSwitch`、`ScopeListBody`、`FilterOptionPopover`。
- 有自然输入输出边界，例如 `v-model` / `defineModel`、`props`、`emits`、`options`、`title`、`count`、`aria-label`。
- 有独立交互或状态，例如选中态、hover / active、滚动、展开收起、disabled、loading、retry、clear。
- 有稳定组件组合，例如 `el-scrollbar` + `el-segmented`、dropdown / popover + options、图标 + 标题 + 操作组。
- 有成组专属样式类，例如 `.deploy-point-segmented*`、`.scope-list-body*`，说明它已经形成局部视觉系统。
- 父组件因此同时承载业务编排和局部控件细节，导致模板、状态或 scoped 样式难以扫读。

不满足独立职责时可以保持内联：纯布局包裹、一次性文案块、没有本地状态 / 交互 / 样式族的小片段，不要为了“显得组件化”制造 props 过多的薄封装。
...
## 抽离前判断
按候选单元逐项判断：
- 独立职责：是否已经有清晰业务语义、输入输出、交互状态和样式族；不要只看是否会被复用。
