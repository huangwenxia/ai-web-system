...
## 新页面开发顺序检查
- 是否先完成需求 / 原型确认，再进入代码实现；目标用户、页面主任务、第一眼判断路径、区块、状态、弹窗、筛选、卡片、表格、空 / 加载 / 错误态是否明确
- 是否从“当前目标用户第一次进入页面”的视角确认页面是否成立；目标用户能否快速理解对象/范围、关键状态/结果/上下文、下一步关注点/动作和动作结果
...
- 可见文案是否已经从内部实现语言翻译成当前用户能理解并能行动的语言；普通业务用户界面是否没有 `Api.general.xxx`、`result.total`、`hasXxx`、`currentStep`、`AI-NOTES`、`data-source`、`mock`、前端路由、源码 API client 名称、状态判断表达式、“数据来源 / 状态判断来源 / 根据规则推导”等无关内部证据
- 如果是开发者/API/SDK/调试/日志/事件追踪页面，是否只展示完成任务所需的接口 URL、方法、Headers、Body、示例、响应、错误码、复制入口或调试信息；敏感信息是否脱敏，低频诊断信息是否折叠或放入详情
...
- 本次新增或实质修改的业务规则、状态流转、接口编排、数据转换、权限与边界判断、异步竞态、副作用和不直观兜底是否都有准确的中文注释；注释是否解释原因、触发条件、约束或结果，而非逐行复述语法
- 简单自解释的导入、类型、常量、直接赋值、模板和样式是否避免机械注释；是否没有借本次任务批量改写未触及的历史代码
...
- 响应式是否由内容驱动换行，优先使用 `flex-wrap`、`flex-basis`、`min-w-0`、`min-w-*`、`max-w-*`、`gap-*`、`ml-auto` 和自然文本换行；若 `check-project-mamba-implementation.mjs` 提示 viewport / clustered breakpoint warning，是否已整改或说明保留的是语义断点
- 高度受限的 flex column 中，标题、摘要、工具条、tabs、底部操作等稳定高度区域是否使用 `shrink-0` / `flex-shrink: 0`；滚动内容区是否使用 `flex-1 min-h-0`
- 固定宽度是否只保护操作区、图标按钮区、操作列等动作区域；业务文本、标题、描述、状态说明和数据内容区是否避免硬设宽度，改用 `min-w-0 flex-1`、换行或合理省略
...
- 是否不存在 `:global(...)` / `:global (...)` 逃逸 scoped 的样式；如发现用 `:global` 覆盖 Element Plus 内部类、浮层壳层或页面外层容器，必须判定未达标并改为局部类、props / wrapper class、`popper-class`、`FormDialog.show` 参数或经批准的共享样式入口
- 内容区宽度变化但 viewport 不变时，row / card / toolbar 是否按自身容器宽度稳定自适应，而不是只靠 `@media` 改结构或用人为断点硬救布局
...
## 输出前检查
- 是否说明了风险与最小验证建议
- 是否完成目标用户 5 秒自检：5 秒内能否理解页面目的、当前状态、下一步关注点/动作和动作结果；未通过时是否先删减、改写和收敛
- 是否确认页面可见 UI 不存在与当前任务无关的代码感、接口感、内部规则感或原型说明感内容
- 是否判断了是否值得回写
