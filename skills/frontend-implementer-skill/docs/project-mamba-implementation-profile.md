...
- 组件能力优先项目已有封装、Element Plus 和 `easybill-ui`；布局、间距和尺寸优先 Tailwind utility；布局结构默认使用 flex，响应式由内容驱动换行，优先通过 `flex-wrap`、`flex-basis`、`min-w-0`、`min-w-*`、`max-w-*`、`gap-*`、`ml-auto` 和自然文本换行处理；稳定高度的标题、摘要、工具条、tabs、底部操作等用 `shrink-0` / `flex-shrink: 0` 防止被纵向压缩；固定宽度只保护操作区、图标按钮区、操作列等动作区域，不给业务文本 / 数据内容硬设宽度；断点只用于布局语义确实变化的少数场景，禁止用多个相近断点硬救布局；禁止新增 Tailwind grid utility 或 CSS Grid 属性；`<style>` 必须 `scoped`，简单布局 / 间距 / 尺寸 / 排版声明优先迁到 template Tailwind class，复杂容器自适应、hover / focus 状态和深层覆盖再放 scoped SCSS；原生 HTML 只用于合适的视觉结构或现有能力缺口。
