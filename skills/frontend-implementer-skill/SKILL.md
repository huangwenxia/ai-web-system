...
- 交付前做“目标用户 5 秒自检”：目标用户能否 5 秒内理解页面目的、当前状态、下一步关注点/动作和动作结果？页面是否存在无关的代码感、接口感、内部规则感或原型说明感内容？未通过则先删减、改写和收敛。
...
12. 语义复查：复查目标用户 5 秒自检、页面入口结构、数据归属组件、胶囊目录、纯工具函数抽离、递归三轮抽离、复用证据、flex / 禁用 grid、Tailwind 优先、可见内部证据清理和边界态完整性。
