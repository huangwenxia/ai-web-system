...
涉及可见 UI 的实现、修复或重构，默认站在“当前目标用户第一次进入页面”的视角判断是否成立。目标用户可能是业务用户、开发者、运维、管理员或集成用户；先判断用户角色、页面任务和信息展示深度，再落代码。

- 简不是内容少，而是用户判断路径短、主次清楚、动作明确。若页面需要解释才能看懂，先收敛主任务、区块职责和业务表达，不继续堆模块、说明、卡片、标签、指标或装饰。
- 首屏必须能让目标用户快速知道正在看什么对象或范围、当前最重要的状态/结果/上下文、下一步自然关注点/动作，以及主操作会进入哪里、改变什么或得到什么结果。
- 页面类型决定重点：列表页看范围、筛选、记录状态和行操作；详情页看对象身份、关键状态、主要配置和关联入口；表单页看填写目标、必填重点和提交后果；流程页看当前步骤、阻塞原因和下一步动作；Dashboard 看整体状态、异常和下钻路径；API/开发者页看接口状态、调用方式、必要参数、复制入口、错误处理和调试路径。
- 客户可见文本不得出现与当前任务无关的内部技术证据，例如 `Api.general.xxx`、`result.total`、`hasXxx`、`currentStep`、`AI-NOTES`、`data-source`、`mock`、前端路由、源码 API client 名称、代码变量、内部判断表达式、“数据来源”“状态判断来源”“根据规则推导”等。
- 如果页面面向开发者/API/SDK/调试/日志/事件追踪，可以展示接口 URL、请求方法、Headers、Body、示例代码、响应示例、错误码和复制入口；但仍不得展示前端源码 API client 名称、页面状态表达式、原型数据来源说明或 AI 设计备注，敏感信息必须脱敏，低频诊断信息应折叠或放进事件详情。
- 内部状态、接口判断和系统规则必须翻译成当前用户能理解并能行动的语言：业务用户看业务状态和下一步动作，开发者看可调用接口、参数、示例和错误处理，管理员看配置影响和风险。
- 交付前做“目标用户 5 秒自检”：目标用户能否 5 秒内理解页面目的、当前状态、下一步关注点/动作和动作结果？页面是否存在无关的代码感、接口感、内部规则感或原型说明感内容？后台页面主内容画布是否被合理利用、右侧大面积空白是否有业务用途？未通过则先删减、改写、调整布局和收敛。
...
1. 需求 / 原型确认：确认目标用户、页面主任务、第一眼判断路径、区块、状态、弹窗、筛选、卡片、表格、空 / 加载 / 错误态；原型不够实施时先补原型，不进代码。
...
12. 语义复查：复查目标用户 5 秒自检、页面入口结构、数据归属组件、胶囊目录、主内容画布利用率、右侧大面积空白业务用途、纯工具函数抽离、递归三轮抽离、复用证据、flex / 禁用 grid、Tailwind 优先、可见内部证据清理和边界态完整性。
...
- 可见文案是否已经从实现细节翻译成目标用户语言；普通业务用户界面不得出现 `Api.general.xxx`、`result.total`、`hasXxx`、`currentStep`、`AI-NOTES`、`data-source`、`mock`、前端路由、源码 API client 名称、状态判断表达式或原型说明感内容。开发者/API/诊断页面只展示完成任务必需的技术信息，并对敏感信息脱敏。
...
- 内部状态、接口判断和系统规则不得直接变成可见文案；业务用户看到业务状态和下一步动作，开发者看到可调用接口、参数、示例和错误处理，管理员看到配置影响和风险。
...
7. 目标用户 5 秒自检结果、主内容画布利用率复查结果，以及是否已清理无关代码感、接口感、内部规则感或原型说明感可见内容。
8. 风险、边界说明与最小验证建议。
9. 是否需要叠加独立审查流。
