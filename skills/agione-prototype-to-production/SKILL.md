...
3. 每页验收门槛：硬编码中文=0 / `any`=0 / 用了共享组件骨架 / 颜色无 raw palette / **§3 停止条件达成（style delta=0 + height match + 并排目检 OK；mismatch% 是参考）** / 中英文 + light/dark 四态正确 / `pnpm --filter <app> tsc` + `pnpm lint` 通过。
