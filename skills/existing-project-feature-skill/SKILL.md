...
涉及可见 UI 的新功能默认站在“当前目标用户第一次进入页面”的视角判断是否成立。目标用户可能是业务用户、开发者、运维、管理员或集成用户；进入实施前必须确认用户角色、页面任务、信息展示深度和第一眼判断路径。
...
6. 校验当前原型是否足以直接实施：
   - 目标用户第一次进入页面时，能否快速判断对象/范围、关键状态/结果/上下文、下一步关注点/动作和主操作结果
   - 页面骨架和区块职责是否清晰
...
- 设计遵循“大道至简”：简不是内容少，而是用户判断路径短、主次清楚、动作明确。页面若需要解释才能看懂，应先收敛主任务和业务表达，不继续增加模块、说明、卡片、标签、指标或装饰。
- 页面类型决定首屏重点：列表页强调范围、筛选、记录状态和行操作；详情页强调对象身份、关键状态、主要配置和关联入口；表单页强调填写目标、必填重点和提交后果；流程页强调当前步骤、阻塞原因和下一步动作；Dashboard 强调整体状态、异常和下钻路径；API/开发者页强调接口状态、调用方式、必要参数、复制入口、错误处理和调试路径。
- 客户可见文本不得出现与当前任务无关的内部技术证据，例如 `Api.general.xxx`、`result.total`、`hasXxx`、`currentStep`、`AI-NOTES`、`data-source`、`mock`、前端路由、源码 API client 名称、状态判断表达式、“数据来源 / 状态判断来源 / 根据规则推导”等。开发者/API/诊断页只展示完成任务所需的技术信息，并对敏感信息脱敏。
...
- 交付前必须做目标用户 5 秒自检：目标用户能否理解页面目的、当前状态、下一步关注点/动作和动作结果；页面是否已清理无关代码感、接口感、内部规则感或原型说明感内容。
...
10. 目标用户 5 秒自检和可见内部证据清理结果。
11. 风险、影响范围与最小验证建议。
12. 是否需要叠加独立审查流。
13. 回写候选与 skill 同步升级建议。
14. 如需回写，明确区分：`skills/`、`rules/`、Claude memory、还是仅保留为当前任务结论。
