...
- 尚未检测：`HEALTH_SNAPSHOT_MISSING`，仅用于历史账号确实没有健康快照的事实，不使用 `STATUS_PENDING`，也不新增后端或前端 `SYNC_PENDING` 状态。状态标签仍显示现有 `ACTION_REQUIRED` 对应的“需处理”；前端仅将该原因码的原因行显示为 warning 黄色，其他 `ACTION_REQUIRED` 的原因行仍为 error，所有需处理原因行均不展示图标。
...
- 无快照账号返回 `ACTION_REQUIRED + IDLE`，原因表达“状态待同步”，并返回现有 `REFRESH_CONNECTIVITY_STATUS` 同步动作；不新增待同步后端或前端状态。状态标签仍显示“需处理”，前端只按 `HEALTH_SNAPSHOT_MISSING` 将原因行显示为 warning 黄色且不展示原因图标，用户仍可使用统一菜单命令“同步状态”。
...
`reasonMessage` 是后端按请求语言生成的最终用户文案。有系统归因且 `reasonParams.providerErrorCode` 或 `reasonParams.providerErrorMessage` 至少一个存在时，后端按本地化模板组合“系统归因：云商证据”，并在存在时追加 `providerRequestId`；没有云商响应证据时只返回系统归因。中文使用全角冒号和“请求 ID”，英文使用半角冒号和“Request ID”，云商错误摘要保留脱敏后的原文。前端只展示 `reasonMessage`，不得读取 `reasonParams` 二次拼接。

`reasonParams` 保留经过脱敏和限长的结构化诊断证据：`providerErrorCode`、`providerErrorMessage`、`providerRequestId`。网络、DNS、TLS、本地配置或程序异常等未取得云商响应的情况不得伪造这些字段；code 和 message 都不存在时不单独展示 request ID。
