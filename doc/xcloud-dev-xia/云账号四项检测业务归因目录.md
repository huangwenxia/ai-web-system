...
本文只负责四个固定检测项的业务归因：统一原因、适用范围、resolution、XCloud 平台事实、云商官方 API 证据、证据成熟度和生产发布状态。
...
1. 收录 XCloud 当前四个检测项范围内，能够由平台权威规则或四家云商结构化 API 证据区分、并且客户可以采取动作的业务原因。
2. 不把所有 SDK 异常、HTTP 状态或云商全局错误码机械转换成业务原因。
3. 不把缺少具体部署上下文时无法证明的地域、工作空间、模型、规格、配额、容量、存储和网络问题提前归到账号配置或账号健康阶段。
...
| `DEPLOYMENT_READINESS` | 部署就绪性 | Deployment readiness | Hashrate 部署上下文与准入编排 + XCloud 云账号能力 + 云商产品 API |
...
`applicability`只声明具体原因适用于哪个检测阶段，以及命中该原因必须具备的账号事实。运行时不生成 `operationCode`或通用 `changeSet`协议；新增或编辑只由请求是否携带 `cloudAccountId`判断。统一结构如下：
...
- 只修改名称时仍执行固定三个检测项；只有本次凭证解析主体与已保存主体不一致时，才命中绑定主体不一致原因。
...
- 部署地域、规格、配额、容量和网络原因只适用于`DEPLOYMENT_ADMISSION + DEPLOYMENT_READINESS`。
...
- 动态内容只通过白名单 `messageParams` 传递；产品名、地域和规格等参数必须来自可信业务字段。
...
三个云商检测项分别使用自己的未归因`reasonCode`，保证每个原因仍只归属一个`checkCode`：
...
DEPLOYMENT_READINESS                → DEPLOYMENT_READINESS_ERROR_UNCLASSIFIED
```
...
截至 2026-07-31：旧 HEAD 已有名称重复、主体解析、部分凭证错误处理和部署前本地校验；xcloud 工作区存在尚未提交的统一检测实现；hashrate HEAD 已有部署提交前校验与实时规格检查。本文未取得四云逐项生产 Fixture，因此可以形成开发目录和上线验证计划，但不能把所有云商映射声称为`OPERATIONALLY_OBSERVED`。

## 5. 统一业务原因总表

以下总表只描述统一原因本身。每条原因都必须显式列出唯一归属的`checkCode`，不得依赖5.1～5.4章节标题推断归因空间。`适用阶段`只出现三个固定`stageCode`；账号配置适用场景只用于说明命中前提，不形成运行时 `operationCode`。云商证据的`evidenceStatus / developmentStatus`只在后续具体映射表中维护，不放入健康快照。
...
部署就绪性必须具有本次部署的地域、工作空间、模型、制品、规格、配额和网络上下文。没有具体部署请求时，这些原因不得出现在账号配置或账号健康结果中。

| `reasonCode` | `checkCode` | 中文 / English | 适用阶段 | 是否可提交前同步预检 | 命中后的检测项结果 | `resolutionCode` |
|---|---|---|---|---|---|---|
| `DEPLOYMENT_PERMISSION_MISSING` | `DEPLOYMENT_READINESS` | 缺少部署权限 / Deployment permission missing | `DEPLOYMENT_ADMISSION` | 是：权限测试或只读操作 | `FAIL` | `VIEW_REQUIRED_PERMISSIONS` |
| `DEPLOYMENT_REGION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 部署地域不可用 / Deployment region unavailable | `DEPLOYMENT_ADMISSION` | 是：官方地域/Location 查询 | `FAIL` | `SELECT_SUPPORTED_REGION` |
| `DEPLOYMENT_WORKSPACE_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 工作空间不可用 / Workspace unavailable | `DEPLOYMENT_ADMISSION` | 是：按ID只读查询 | `FAIL` | `CONFIGURE_WORKSPACE` |
| `DEPLOYMENT_MODEL_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 模型不可用 / Model unavailable | `DEPLOYMENT_ADMISSION` | 是：本地模型/版本与云端只读查询 | `FAIL` | `SELECT_AVAILABLE_MODEL` |
| `DEPLOYMENT_ARTIFACT_ACCESS_DENIED` | `DEPLOYMENT_READINESS` | 模型制品不可访问 / Model artifact access denied | `DEPLOYMENT_ADMISSION` | 部分：仅可验证调用者读取；服务角色代入读取通常不能无副作用证明 | `FAIL` | `UPDATE_ARTIFACT_ACCESS` |
| `DEPLOYMENT_SPECIFICATION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 部署规格不可用 / Deployment specification unavailable | `DEPLOYMENT_ADMISSION` | 是：规格目录/机器类型查询 | `FAIL` | `SELECT_AVAILABLE_SPECIFICATION` |
| `DEPLOYMENT_QUOTA_INSUFFICIENT` | `DEPLOYMENT_READINESS` | 部署配额不足 / Deployment quota insufficient | `DEPLOYMENT_ADMISSION` | 部分：云商提供可比较配额读接口时 | `FAIL` | `REQUEST_QUOTA` |
| `DEPLOYMENT_CAPACITY_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 实时容量不足 / Deployment capacity unavailable | `DEPLOYMENT_ADMISSION` | 部分：有官方实时库存接口时；否则只能创建阶段观察 | `FAIL` | `RETRY_OR_CHANGE_CAPACITY_SCOPE` |
| `DEPLOYMENT_NETWORK_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 部署网络不可用 / Deployment network unavailable | `DEPLOYMENT_ADMISSION` | 是：VPC/子网/安全组/Endpoint只读查询；端到端连通性通常不能完全前置 | `FAIL` | `UPDATE_NETWORK_CONFIGURATION` |
| `DEPLOYMENT_SERVICE_ROLE_MISSING` | `DEPLOYMENT_READINESS` | 服务角色缺失 / Service role missing | `DEPLOYMENT_ADMISSION` | 是：角色/Agency按名只读查询 | `FAIL` | `CONFIGURE_SERVICE_ROLE` |
| `DEPLOYMENT_SERVICE_ROLE_INVALID` | `DEPLOYMENT_READINESS` | 服务角色无效 / Invalid service role | `DEPLOYMENT_ADMISSION` | 部分：信任策略和附加权限可读时 | `FAIL` | `REPAIR_SERVICE_ROLE` |
| `DEPLOYMENT_RESOURCE_CONFLICT` | `DEPLOYMENT_READINESS` | 部署资源冲突 / Deployment resource conflict | `DEPLOYMENT_ADMISSION` | 是：名称确定且有按名查询时 | `FAIL` | `CHANGE_DEPLOYMENT_RESOURCE_NAME` |
| `DEPLOYMENT_CONFIGURATION_INVALID` | `DEPLOYMENT_READINESS` | 部署配置无效 / Invalid deployment configuration | `DEPLOYMENT_ADMISSION` | 是：本地契约、框架/云/地域组合；云商深层校验视API能力 | `FAIL` | `UPDATE_DEPLOYMENT_CONFIGURATION` |
| `DEPLOYMENT_READINESS_ERROR_UNCLASSIFIED` | `DEPLOYMENT_READINESS` | 部署就绪错误暂未归因 / Unclassified deployment readiness error | `DEPLOYMENT_ADMISSION` | 不是主动探针；仅承载明确业务失败的安全回落 | `FAIL` | — |

规格不可用表示目标范围本身不支持该规格；容量不足表示规格受支持但当前暂时没有容量。二者的证据和resolution必须分开。
...
- `metis-hashrate`HEAD：`adb8e97f140e1579f0dd668381df08bbd00ba793`；核验时工作区干净。

| 事实 | 旧 HEAD 已有 | 当前未提交实现/缺口 | 目录结论 |
|---|---|---|---|
| xcloud新增/编辑云账号 | 旧 HEAD 有名称重复查询、云商`validateAccount`调用、账号主体回填与编辑主体变化拦截；但`CloudAccountServiceImpl.validate(...)`已被注释为返回空VO，新增/编辑链路并未形成统一四项检测契约 | 工作区新增健康快照、分类器、检测服务与保存编排；仍需删除Token和旧顶层执行状态 | 保存命令复核最终提交数据；账号信息/身份失败不保存，产品访问业务失败可保存并初始化为“需处理”；不增加候选版本或新前端流程 |
| xcloud云商身份 | 旧 HEAD真实适配器：阿里云、阿里金融云、AWS、Google Cloud、华为云、Infracube；阿里云用BSS账单概览，AWS用STS `GetCallerIdentity`，Google解析Service Account并刷新token，华为用IAM Domain，Infracube调用私有平台登录 | 工作区增强了部分结构化错误采集，但尚未提交，不能写成旧能力 | 分别登记`CODE_REPRODUCED`或`SDK_OBSERVABLE`，不能统一称“已上线四项健康检测” |
| runStatus/健康列表 | 旧 HEAD没有统一DetectionRun | 工作区枚举和对象仍需按架构收敛 | `runStatus`仅`RUNNING/COMPLETED`；技术失败进入对应item的`ERROR`；列表只投影健康/需处理/检测中 |
| hashrate部署入口 | HEAD在`CloudInferenceJobServiceImpl.create`中依次校验License、部署模式/路由、模型与框架版本、预创建任务、实时规格可用性，再异步创建云服务 | 已有同步上下文校验和真实部署异步错误链路，不新增统一部署预检接口 | 保留hashrate为准入Owner；已有同步校验继续在真实部署命令内执行，真实创建错误按`DEPLOYMENT_READINESS`归因 |
| hashrate许可证 | HEAD调用`remoteLicenseService.requireCreateAllowed(AI_INFRA, 1)` | 只检查本次创建1条运行实例，需确认与实例数量业务定义一致 | 属于hashrate本地同步准入，不是云商归因；统一放入`DEPLOYMENT_CONFIGURATION_INVALID`或独立Stage Policy提示，不交给xcloud |
| hashrate模型/框架/必填 | HEAD校验模型ID、云类型、地域、框架版本ID，查询模型与框架版本；确认推荐部署时校验框架类型、云类型、地域一致性 | 传统`create`路径的组合校验少于`confirmDeploy`路径，`getModelFrameworkVersion`返回空时可能到异步阶段才暴露 | 保持在真实部署命令中完善确定性校验，不新增独立预检接口，固定归属`DEPLOYMENT_READINESS` |
| hashrate账号 | HEAD查询账号存在，校验账号云类型；高可用确认路径检查每条路由云类型至少有租户账号 | 普通创建和高可用异步激活仍有“找不到则跳过”的路径，未统一读取账号健康快照 | Hashrate单向调用xcloud `hyperone-account-api`取得账号与最近有效健康结论；不得反向依赖hashrate |
| hashrate规格 | HEAD调用`InferenceJobComponent.checkFlavorAvailability`，可在云端创建前发现不可用；但调用发生在本地任务预创建之后 | 各云商实现能力和证据结构不一致，返回自由文本时无法精确区分规格不支持、配额或瞬时容量 | 保留在真实部署流程；能结构化区分时精确归因，不能区分时回落`DEPLOYMENT_READINESS`未归因 |

### 6.2 所有权与调用方向

```text
hashrate（模型、框架、地域、规格、路由、License、最终部署按钮）
    └─ 单向调用 xcloud hyperone-account-api
       （云账号、AK/SK、主体、基础产品访问、最近有效健康结论）
```

`DEPLOYMENT_ADMISSION`归因空间、上下文聚合和真实部署流程归Hashrate；XCloud不得保存模型部署状态或反向调用Hashrate。当前不新增XCloud或Hashrate部署预检接口，统一的是`stageCode/checkCode/归因`目录，不是新增公共编排服务。

### 6.3 保存与重检固定决策

- 取消`precheckToken`。新增/编辑保存命令对最终将要持久化的数据完成必要复核，不能信任前端回传检测结果。
- 不引入HMAC、候选版本状态机、配置版本或新前端流程。
- 配置阶段固定返回账号信息、身份凭证、产品访问三个检测项；未改变的敏感字段使用服务端原值合并后再检测，不能把前端掩码当真实凭证。
...
|---|---|---|---|---|
| `ALIBABA_FINANCE_CLOUD` | `CLOUD_IDENTITY_CREDENTIALS` | `AlifinAccountComponent`先调用BSS `QueryBillOverview`，再用ECS `DescribeRegions`判断是否出现金融云地域 | 阿里金融云仍使用阿里云官方OpenAPI；金融云地域和Endpoint必须按官方地域/网络文档配置，不能因普通地域列表为空就判定凭证错误 | 复用阿里云凭证归因；另以`CREDENTIAL_PROJECT_MISMATCH`不合适，金融云类型不匹配应先作为`DEPLOYMENT_REGION_UNAVAILABLE`或本地云类型校验，待冻结专用原因后再扩展 |
...
`Platform`枚举中出现腾讯云、Azure、百度云、火山云、华为云Stack等，不等于xcloud云账号新增/编辑链路已有对应`CloudAccountComponent`。在旧 HEAD 没有适配器和契约证据前，不把枚举值列为已支持账号检测。hashrate中存在腾讯云、天翼云、VirtualCloud等部署插件，也不自动证明xcloud已拥有这些云账号身份与健康能力；若要纳入，先补xcloud能力Owner和官方/私有平台契约，再进入本目录。
...
## 9. `DEPLOYMENT_ADMISSION`归因目录

本章用于Hashrate真实部署流程的系统归因，不代表新增独立同步预检接口。核对结论是：实际部署就绪性原因均依赖部分或完整部署上下文；因此不在XCloud统一检测接口实现`DEPLOYMENT_ADMISSION`，也不在Hashrate新增另一套部署预检接口。Hashrate已有同步上下文校验继续在最终部署命令中执行，真实创建及异步执行错误继续按本章目录归因。

### 9.1 hashrate HEAD 已有同步检查

| 检查 | `stageCode` | `checkCode` | 当前时点 | 代码事实与应收敛项 |
|---|---|---|---|---|
| License实例数 | `DEPLOYMENT_ADMISSION` | `DEPLOYMENT_READINESS` | 云端创建前 | `requireCreateAllowed(AI_INFRA, 1)`；保持hashrate所有，不调用xcloud |
| 部署模式/主节点/高可用策略必填 | `DEPLOYMENT_ADMISSION` | `DEPLOYMENT_READINESS` | Controller与Service同步 | 已有；应统一由Service准入器返回稳定归因，Controller只保留参数绑定 |
| 模型存在、框架版本必填/存在 | `DEPLOYMENT_ADMISSION` | `DEPLOYMENT_READINESS` | 本地任务预创建前 | 已有；需补空框架版本的确定性失败，不能进入异步空指针 |
| 账号存在、账号与云类型一致 | `DEPLOYMENT_ADMISSION` | `DEPLOYMENT_READINESS` | 推荐确认与规格检查时 | 已有；普通创建/推荐确认应共用同一检查，并读取xcloud最近有效健康结论 |
| 框架类型/云类型/地域一致 | `DEPLOYMENT_ADMISSION` | `DEPLOYMENT_READINESS` | `confirmDeploy`同步 | 已有；传统`create`路径也必须执行 |
| 高可用每条路由存在同云类型租户账号 | `DEPLOYMENT_ADMISSION` | `DEPLOYMENT_READINESS` | `confirmDeploy`同步 | 已有；不能在异步激活时“找不到就跳过”后继续部署 |
| 云规格实时可用 | `DEPLOYMENT_ADMISSION` | `DEPLOYMENT_READINESS` | 当前在本地任务预创建后、云端创建前 | 只有阿里EAS真正覆盖`DescribeMachineSpec`；AWS/Google/华为覆盖方法调用默认实现，`realTimeChecked=false`且直接可用，不能声称已实时检查 |

### 9.2 通用部署就绪归因项

表中每条归因均固定`stageCode=DEPLOYMENT_ADMISSION`、`checkCode=DEPLOYMENT_READINESS`。其中“提交前同步”表示当真实部署命令已经取得所需上下文时，可以在调用云商Create之前执行；它不是无上下文账号预检，也不产生新接口。

| `reasonCode` | `checkCode` | 提交前同步 | 最早可检阶段 | 所需输入 | 只读无副作用探针 | 不能前置/剩余边界 | 用户提示与处理操作 | `structuredEvidence` |
|---|---|---|---|---|---|---|---|---|
| `DEPLOYMENT_PERMISSION_MISSING` | `DEPLOYMENT_READINESS` | 是 | 账号、地域、目标产品和操作集合确定后 | provider、账号、region、workspace/project、目标操作 | 权限测试API；没有测试API时调用最小只读List/Get | 只读权限通过不保证Create权限；普通403不伪造具体权限 | “缺少部署所需权限”；展示缺失Action并进入云商权限配置 | `missingPermissions[]`或操作级结构化拒绝 |
| `DEPLOYMENT_REGION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 是 | provider、产品和region确定后 | provider、product、region | 官方Regions/Locations或产品地域目录 | 运行时区域临时故障不等于不支持 | “当前地域不支持部署”；重新选择支持地域 | `supportedRegions[]`不含目标值 |
| `DEPLOYMENT_WORKSPACE_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 是 | workspace/project已选择 | region、workspaceId/projectId | 按IDGet/List并精确匹配 | 未填写属于参数校验；列表空不能证明产品未开通 | “工作空间不存在或不可访问”；选择/配置工作空间 | `resourceFound=false`或明确NotFound/Denied |
| `DEPLOYMENT_MODEL_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 是 | model、frameworkVersion、region确定后 | 本地modelId/versionId、云端modelId（如有） | 本地权威查询；云端Models Get/List | 刚导入模型可能最终一致；创建时的制品解包错误不能前置 | “模型或版本当前不可部署”；选择可用版本/等待同步 | 本地状态、云端存在性和部署状态字段 |
| `DEPLOYMENT_ARTIFACT_ACCESS_DENIED` | `DEPLOYMENT_READINESS` | 部分 | artifact URI和执行角色确定后 | bucket/object/image URI、role/agency | Head/Get metadata、镜像Describe/BatchGet | 当前调用者能读不等于云服务执行角色能读；不得下载大文件 | “部署服务无法读取模型制品”；修复对象/镜像与角色权限 | 明确对象/镜像NotFound或AccessDenied，带资源摘要 |
| `DEPLOYMENT_SPECIFICATION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 是 | region、模型框架、spec、加速器确定后 | region、specId、accelerator、instanceCount | 规格/机器类型目录 | 目录支持不等于实时有库存 | “该地域不支持所选规格”；更换规格或地域 | 规格不存在、状态禁用或组合不兼容字段 |
| `DEPLOYMENT_QUOTA_INSUFFICIENT` | `DEPLOYMENT_READINESS` | 部分 | 资源需求可计算后 | quotaDimension、currentUsage、requested、limit | 配额Get/List | 云商不暴露当前用量或维度无法对齐时不能精确前置 | “当前配额不足”；申请配额或减少实例数 | `usage + requested > limit`及单位/维度 |
| `DEPLOYMENT_CAPACITY_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 部分 | region、zone、spec、count确定后 | region/zone/spec/count | 官方实时库存/可用性查询 | 无实时库存API时只能创建阶段观察；查询后仍有竞态 | “当前规格暂时无容量”；重试或更换地域/规格 | `supported=true`且`available=false/stockStatus` |
| `DEPLOYMENT_NETWORK_UNAVAILABLE` | `DEPLOYMENT_READINESS` | 部分 | VPC/子网/安全组/endpoint输入齐全 | 网络资源ID、region、必要端口 | VPC/Subnet/SecurityGroup/Endpoint Get/List | 安全组规则语义、DNS、路由和服务托管网络的端到端可达性未必可无副作用证明 | “网络配置不可用”；选择有效网络或修正规则 | 存在性、状态、region归属、明确拒绝字段 |
| `DEPLOYMENT_SERVICE_ROLE_MISSING` | `DEPLOYMENT_READINESS` | 是 | 云商和角色名/Agency确定后 | roleArn/agencyName/project | IAM Role/Agency Get | 角色自动创建流程尚未完成时应提示配置，不擅自创建 | “缺少部署服务角色”；配置服务角色 | NotFound或空精确匹配 |
| `DEPLOYMENT_SERVICE_ROLE_INVALID` | `DEPLOYMENT_READINESS` | 部分 | 角色存在后 | role、信任主体、所需权限 | GetRole/GetAgency、策略查询/权限测试 | AssumeRole/服务代入的最终行为仍可能只在创建阶段暴露 | “服务角色信任或授权无效”；修复信任与权限 | trustPolicy、attachedPermissions、agency status |
| `DEPLOYMENT_RESOURCE_CONFLICT` | `DEPLOYMENT_READINESS` | 是 | 最终云端资源名可确定后 | endpoint/service/model/config name | 按名称Describe/List | 云商可能在检查后被并发创建 | “同名部署资源已存在”；改名或选择复用 | 精确名称和资源类型已存在 |
| `DEPLOYMENT_CONFIGURATION_INVALID` | `DEPLOYMENT_READINESS` | 是 | 表单全部字段齐全后 | model/framework/cloud/region/spec/count/route/必填参数 | 本地契约和组合矩阵；云商Validate API（仅在明确无副作用时） | 云商Create深层校验没有Dry Run时仍会晚失败 | “部署配置不兼容”；定位字段并修改 | `field/path/constraint/actual`，禁止自由文本猜测 |

### 9.3 四云具体探针与官方证据

| provider | `reasonCode` | `checkCode` | `probeApi` / `providerOperation` | 所需权限 | `structuredEvidence` | 成熟度与开发项 |
|---|---|---|---|---|---|---|
| `ALIBABA_CLOUD`/`ALIBABA_FINANCE_CLOUD` | `DEPLOYMENT_PERMISSION_MISSING` | `DEPLOYMENT_READINESS` | PAI `ListWorkspaces`；EAS `ListServices` | `paiworkspace:ListWorkspaces`、`eas:ListServices` | 操作级`AccessDenied`、RequestId、Action | `OFFICIALLY_DOCUMENTED + PRODUCTION_VALIDATION_REQUIRED`；实现只读适配器 |
| `ALIBABA_CLOUD`/`ALIBABA_FINANCE_CLOUD` | `DEPLOYMENT_REGION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | ECS `DescribeRegions` + PAI/EAS地域配置 | `ecs:DescribeRegions`及产品只读权限 | 目标region不在产品支持集合；金融云必须使用金融云地域集合 | 旧HEAD阿里金融云已调用`DescribeRegions`，但吞并了具体错误；需结构化保留 |
| `ALIBABA_CLOUD`/`ALIBABA_FINANCE_CLOUD` | `DEPLOYMENT_SPECIFICATION_UNAVAILABLE`/`DEPLOYMENT_CAPACITY_UNAVAILABLE` | `DEPLOYMENT_READINESS` | EAS `DescribeMachineSpec` | EAS对应只读Action | `instanceType`存在性、`isAvailable`、`stockStatus=WithStock` | `CODE_REPRODUCED + SDK_OBSERVABLE`；hashrate HEAD已有真实覆盖，应前移至预创建前并区分“不支持/无库存” |
| `ALIBABA_CLOUD`/`ALIBABA_FINANCE_CLOUD` | `DEPLOYMENT_WORKSPACE_UNAVAILABLE` | `DEPLOYMENT_READINESS` | PAI Workspace Get/List | Workspace读取权限 | 指定workspace精确NotFound/Denied | 可同步；空列表不能映射产品未开通 |
| `AWS` | `DEPLOYMENT_PERMISSION_MISSING` | `DEPLOYMENT_READINESS` | IAM `SimulatePrincipalPolicy`；降级SageMaker只读List/Describe | `iam:SimulatePrincipalPolicy`（若允许）及目标只读Action | `EvalDecision != allowed`、`MissingContextValues`；或操作级`AccessDeniedException` | 官方确认；模拟权限自身常被限制，需保留降级与未知回落 |
| `AWS` | `DEPLOYMENT_REGION_UNAVAILABLE`/`DEPLOYMENT_SPECIFICATION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | SageMaker支持地域与`ListInferenceRecommendationsJobSteps`等能力目录；本地经审核的SDK region元数据 | 对应只读Action | region/instanceType不在明确支持集合 | 需要按当前SageMaker托管推理类型选具体API，不能用EC2机型目录冒充SageMaker支持规格 |
| `AWS` | `DEPLOYMENT_QUOTA_INSUFFICIENT` | `DEPLOYMENT_READINESS` | Service Quotas `GetServiceQuota`/`ListServiceQuotas`，serviceCode=`sagemaker` | `servicequotas:GetServiceQuota`/`ListServiceQuotas` | quotaCode、value、usage（若另有指标）、requested、unit | 限额可读；当前用量需本地资源计数或CloudWatch/列表，维度对不齐则不精确归因 |
| `AWS` | `DEPLOYMENT_SERVICE_ROLE_MISSING/INVALID` | `DEPLOYMENT_READINESS` | IAM `GetRole`、`GetRolePolicy`、`ListAttachedRolePolicies`、可选`SimulatePrincipalPolicy` | 对应IAM只读/模拟权限 | role存在、信任主体、所需Action决策 | 可同步检查存在性与可见策略；最终`iam:PassRole`/SageMaker代入仍可能创建时失败 |
| `AWS` | `DEPLOYMENT_ARTIFACT_ACCESS_DENIED` | `DEPLOYMENT_READINESS` | S3 `HeadObject`；ECR `BatchGetImage` | `s3:GetObject`、`ecr:BatchGetImage` | 结构化NotFound/AccessDenied、bucket/key或repository digest摘要 | 只证明当前身份读取；执行角色读取需结合角色策略 |
| `GOOGLE_CLOUD` | `DEPLOYMENT_PERMISSION_MISSING` | `DEPLOYMENT_READINESS` | Resource Manager/Vertex资源`testIamPermissions`；Service Usage `services.get` | `resourcemanager.projects.getIamPolicy`并非所有test都必需，以目标资源文档为准；`serviceusage.services.get/use` | 返回的`permissions[]`与请求集合差集；`google.rpc.ErrorInfo` metadata | 官方确认；`testIamPermissions`只返回调用者拥有的权限，不保证未来Create无竞态 |
| `GOOGLE_CLOUD` | `DEPLOYMENT_REGION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | Vertex AI `projects.locations.list/get` | `aiplatform.locations.list/get`（按官方IAM角色映射） | 目标Location缺失或明确不支持目标能力 | 可同步；Location存在不等于指定模型/机器类型可用 |
| `GOOGLE_CLOUD` | `DEPLOYMENT_MODEL_UNAVAILABLE` | `DEPLOYMENT_READINESS` | Vertex AI `projects.locations.models.get/list` | `aiplatform.models.get/list` | Model不存在、状态或region不匹配 | 自定义模型可同步；Model Garden模型需使用对应官方目录接口 |
| `GOOGLE_CLOUD` | `DEPLOYMENT_SPECIFICATION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | Vertex AI machine types/locations能力文档与`projects.locations.machineTypes.list`（SDK实际支持时） | 对应只读权限 | machineType/accelerator组合不在返回集合 | hashrate当前覆盖方法是默认放行，不是实时检查；需新增真实适配器 |
| `GOOGLE_CLOUD` | `DEPLOYMENT_QUOTA_INSUFFICIENT` | `DEPLOYMENT_READINESS` | Cloud Quotas `projects.locations.quotas.get/list`或官方Vertex配额指标 | `cloudquotas.quotas.get/list` | metric、dimensions、effectiveLimit、usage/requested | API与项目启用情况需生产验证；通用`RESOURCE_EXHAUSTED`无quota metadata时不精确归因 |
| `HUAWEI_CLOUD` | `DEPLOYMENT_PERMISSION_MISSING` | `DEPLOYMENT_READINESS` | ModelArts在线服务List/Get及授权查询 | `modelarts:service:list`、`modelarts:authorization:list` | `ModelArts.0108/4336/6653`仅在选定只读接口实际返回时 | 官方错误码目录确认，操作级仍需生产验证 |
| `HUAWEI_CLOUD` | `DEPLOYMENT_WORKSPACE_UNAVAILABLE` | `DEPLOYMENT_READINESS` | ModelArts Workspace查询 | Workspace读取权限 | workspace精确NotFound/Denied、project/region不匹配 | 可同步；全局错误码不能脱离接口使用 |
| `HUAWEI_CLOUD` | `DEPLOYMENT_SERVICE_ROLE_MISSING/INVALID` | `DEPLOYMENT_READINESS` | ModelArts授权管理 + IAM Agency查询 | `modelarts:authorization:list`及IAM Agency只读权限 | `ModelArts.2785`、`4335`或Agency结构化状态 | 官方确认，需验证当前SDK/API版本真实响应 |
| `HUAWEI_CLOUD` | `DEPLOYMENT_SPECIFICATION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | ModelArts资源规格/在线服务配置查询 | 对应ModelArts只读权限 | flavor不存在/不支持region或模型类型 | hashrate当前覆盖方法默认放行；需新增真实只读适配器 |

### 9.4 只能在真实创建或创建后异步确认的项目

| 项目 | 为什么不能可靠前置 | 运行时处理 |
|---|---|---|
| 查询后发生的配额/库存竞态 | 预检与Create之间状态可变 | Create结构化配额/容量错误仍归`DEPLOYMENT_READINESS`，标明`observedAt=CREATE`并给出重试/换规格；不得承诺预检消灭竞态 |
| 云商最终一致性 | 新建角色、授权、模型、镜像或对象可能尚未传播 | 提示等待并重试；保留provider requestId和安全结构化状态 |
| 托管服务实际代入角色读取制品 | 当前凭证Head/Get成功不等于云服务执行身份成功 | 在Create或异步状态失败时依据明确角色/制品错误归因；一般失败不猜测 |
| 容器启动、模型解包、健康检查、端口/命令错误 | 需要真实运行环境、镜像和模型加载 | 归部署创建后的异步错误；若字段契约可本地验证则前置一部分，其余保留任务错误事件 |
| VPC路由、DNS、NAT、服务端点端到端连通 | 只读资源存在性不足以证明数据面路径 | 创建后健康/事件观察；网络超时进入当前item的`ERROR`，只有云商明确网络配置错误才业务归因 |
| 云商内部故障、限流、5xx | 用户无法通过改配置确定解决 | 进入实际执行项的`ERROR`；账号健康保留上次有效结论，部署本次失败关闭，不映射业务原因 |

### 9.5 官方来源

- 阿里云：[PAI ListWorkspaces](https://www.alibabacloud.com/help/en/pai/developer-reference/api-aiworkspace-2021-02-04-listworkspaces)、[EAS ListServices](https://www.alibabacloud.com/help/en/pai/developer-reference/api-eas-2021-07-01-listservices)、[EAS CreateService](https://www.alibabacloud.com/help/en/pai/developer-reference/api-eas-2021-07-01-createservice)、[EAS错误码](https://api.alibabacloud.com/document/eas/2021-07-01/errorCode)、[ECS DescribeRegions](https://www.alibabacloud.com/help/en/ecs/developer-reference/api-ecs-2014-05-26-describeregions)。
- AWS：[STS GetCallerIdentity](https://docs.aws.amazon.com/STS/latest/APIReference/API_GetCallerIdentity.html)、[IAM SimulatePrincipalPolicy](https://docs.aws.amazon.com/IAM/latest/APIReference/API_SimulatePrincipalPolicy.html)、[SageMaker CreateEndpoint](https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_CreateEndpoint.html)、[Service Quotas GetServiceQuota](https://docs.aws.amazon.com/servicequotas/2019-06-24/apireference/API_GetServiceQuota.html)、[IAM GetRole](https://docs.aws.amazon.com/IAM/latest/APIReference/API_GetRole.html)、[S3 HeadObject](https://docs.aws.amazon.com/AmazonS3/latest/API/API_HeadObject.html)。
- Google Cloud：[Service Usage services.get](https://cloud.google.com/service-usage/docs/reference/rest/v1/services/get)、[IAM testIamPermissions](https://cloud.google.com/iam/docs/testing-permissions)、[Vertex AI Locations](https://cloud.google.com/vertex-ai/docs/reference/rest/v1/projects.locations)、[Vertex AI Models get](https://cloud.google.com/vertex-ai/docs/reference/rest/v1/projects.locations.models/get)、[Vertex AI deployModel](https://cloud.google.com/vertex-ai/docs/reference/rest/v1/projects.locations.endpoints/deployModel)、[Vertex AI quotas](https://cloud.google.com/vertex-ai/docs/quotas)、[Cloud Quotas API](https://cloud.google.com/docs/quotas/reference/rest)。
- 华为云：[ModelArts API](https://support.huaweicloud.com/intl/en-us/api-modelarts/)、[ModelArts错误码](https://support.huaweicloud.com/intl/en-us/api-modelarts/modelarts_03_0095.html)、[ModelArts权限](https://support.huaweicloud.com/api-modelarts/modelarts_03_0146.html)、[ModelArts授权管理](https://support.huaweicloud.com/api-modelarts/modelarts_03_0167.html)、[ModelArts Workspace权限](https://support.huaweicloud.com/api-modelarts/modelarts_03_0166.html)。
...
| `DEPLOYMENT_PERMISSION_MISSING` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentPermissionMissing` |
| `DEPLOYMENT_REGION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentRegionUnavailable` |
| `DEPLOYMENT_WORKSPACE_UNAVAILABLE` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentWorkspaceUnavailable` |
| `DEPLOYMENT_MODEL_UNAVAILABLE` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentModelUnavailable` |
| `DEPLOYMENT_ARTIFACT_ACCESS_DENIED` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentArtifactAccessDenied` |
| `DEPLOYMENT_SPECIFICATION_UNAVAILABLE` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentSpecificationUnavailable` |
| `DEPLOYMENT_QUOTA_INSUFFICIENT` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentQuotaInsufficient` |
| `DEPLOYMENT_CAPACITY_UNAVAILABLE` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentCapacityUnavailable` |
| `DEPLOYMENT_NETWORK_UNAVAILABLE` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentNetworkUnavailable` |
| `DEPLOYMENT_SERVICE_ROLE_MISSING` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentServiceRoleMissing` |
| `DEPLOYMENT_SERVICE_ROLE_INVALID` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentServiceRoleInvalid` |
| `DEPLOYMENT_RESOURCE_CONFLICT` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentResourceConflict` |
| `DEPLOYMENT_CONFIGURATION_INVALID` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentConfigurationInvalid` |
| `DEPLOYMENT_READINESS_ERROR_UNCLASSIFIED` | `DEPLOYMENT_READINESS` | `cloudAccount.detection.reason.deploymentReadinessErrorUnclassified` |
...
| `SELECT_SUPPORTED_REGION` | `cloudAccount.detection.resolution.selectSupportedRegion` | 选择支持的地域 / Select a supported region |
| `CONFIGURE_WORKSPACE` | `cloudAccount.detection.resolution.configureWorkspace` | 配置工作空间 / Configure workspace |
| `SELECT_AVAILABLE_MODEL` | `cloudAccount.detection.resolution.selectAvailableModel` | 选择可用模型 / Select an available model |
| `UPDATE_ARTIFACT_ACCESS` | `cloudAccount.detection.resolution.updateArtifactAccess` | 更新模型制品权限 / Update model artifact access |
| `SELECT_AVAILABLE_SPECIFICATION` | `cloudAccount.detection.resolution.selectAvailableSpecification` | 选择可用规格 / Select an available specification |
| `REQUEST_QUOTA` | `cloudAccount.detection.resolution.requestQuota` | 申请配额 / Request quota |
| `RETRY_OR_CHANGE_CAPACITY_SCOPE` | `cloudAccount.detection.resolution.retryOrChangeCapacityScope` | 重试或更换地域/规格 / Retry or change region/specification |
| `UPDATE_NETWORK_CONFIGURATION` | `cloudAccount.detection.resolution.updateNetworkConfiguration` | 更新网络配置 / Update network configuration |
| `CONFIGURE_SERVICE_ROLE` | `cloudAccount.detection.resolution.configureServiceRole` | 配置服务角色 / Configure service role |
| `REPAIR_SERVICE_ROLE` | `cloudAccount.detection.resolution.repairServiceRole` | 修复服务角色 / Repair service role |
| `CHANGE_DEPLOYMENT_RESOURCE_NAME` | `cloudAccount.detection.resolution.changeDeploymentResourceName` | 修改资源名称 / Change resource name |
| `UPDATE_DEPLOYMENT_CONFIGURATION` | `cloudAccount.detection.resolution.updateDeploymentConfiguration` | 修改部署配置 / Update deployment configuration |
