...
- 前端对接文档：[云账号前端常量与操作对接协议](./云账号前端常量与操作对接协议.md)
...
| 只改名称 | `accountNameChanged=true` | 执行账号信息；身份凭证和云产品访问固定返回 `SKIPPED / NOT_REQUIRED`，不调用云商 |
| 更新凭证或凭证相关扩展 | `credentialsChanged=true`或相关配置变化 | 执行账号信息、身份凭证和云产品访问 |
| 名称与凭证同时修改 | 两类变化同时为真 | 两类检测全部执行，成功后原子保存 |
| 无有效变化 | `effectiveChange=false` | 三个响应槽位仍固定存在；不调用云商、不重复持久化，保持既有成功响应 |

前项失败导致后项无法执行时，后项返回 `SKIPPED / BLOCKED_BY_PREVIOUS_CHECK`；因本次编辑未触及对应能力而不需要执行时，返回 `SKIPPED / NOT_REQUIRED`。两种情况都不能假装 `PASS`。

编辑请求使用局部更新语义：字段未出现表示保持原值，`properties: []`表示明确清空普通属性；普通扩展字段显式为 `null`表示清除该字段。主凭证及扩展凭证只写不读，未传或传 `******`表示沿用旧值，只有完整提交一组新凭证才执行轮换和重新检测；半组、空字符串或 `null`凭证必须拒绝。后端请求模型必须能区分“字段未出现”和“明确传入空值”，不得仅以反序列化后的 `null`猜测用户意图。

华为扩展凭证 `ext.masterKey / ext.masterSecret`与主 `key / secret`遵循相同安全边界：响应中 `masterKey`只可返回掩码，`masterSecret`不得返回；通用 `ext`不得成为绕过后端脱敏的明文出口。
...
| `SKIPPED` | 前项阻断、当前变更不需要执行或能力未实现 | 无 | `skipReason` |

技术失败仍放在发生它的检测项中，以 `stageCode + checkCode`标记执行空间，但不能包装为凭证错误、权限不足等业务原因。技术错误码必须表达经过脱敏的真实原因，不得把所有异常压缩为笼统的“请求失败”或“内部错误”。至少区分连接超时、读取超时、DNS解析失败、云商服务不可用、限流、代理不可用、TLS握手失败、检测配置缺失、云商响应异常和真正的内部错误。
...
动态业务原因、技术错误和处理操作由后端按照请求语言生成可直接展示的 `reasonMessage / message / action.label`。列表快照对应返回 `reasonMessage / actionLabel / lastAttemptMessage / lastAttemptActionLabel`。代码与消息键仍作为稳定机器协议和后端内部存储依据，前端不得再次翻译动态文案；前端只维护健康主状态、固定检测项、检测结果等少量稳定UI常量。
...
- 记录并向列表补充最近一次技术尝试；它不是第四种健康状态，而是旧有效结论旁的辅助错误信息。

列表辅助错误必须包含发生错误的云商、检测项、具体脱敏原因、发生时间、是否可重试和处理操作。例如“连接 Google Cloud 超时；当前保留上次有效结论”，不能只显示“最近一次检测失败”。手动检测与定时检测使用同一套原因分类和展示数据。

列表主时间仍展示“最近更新时间”，使用云账号实体已有 `updateTime`，为空时回退 `createTime`；最近检测尝试时间仅随辅助错误信息展示，不替代列表主时间。
...
5. 健康检测开始时保存后端读取到的相关配置快照；写回前在事务内重新读取并锁定账号。若与检测相关的配置已经变化，丢弃旧任务结果。不新增前端版本参数，也不要求前端参与并发控制。
...
产品探针地域使用后端环境或平台能力配置中的基准地域，不在 Java 代码中写死，也不允许前端任意传入。账号健康结论只证明该基准地域的基础产品访问能力，不代表所有地域都可用；具体业务在目标地域的准入由对应使用流程负责。

Google检测必须同时证明 Vertex AI API 已启用，以及同一服务账号能够在配置的基准地域执行真实、安全、只读的 Vertex AI 最小查询。只检查 Service Usage 的启用状态不能判为产品权限 `PASS`。Google代理为可选后端配置，禁止固定依赖 `127.0.0.1:7890`。
...
- 不用自由文本猜测业务原因；
- 未实现能力显式返回未执行或技术错误。
- Provider原始响应、异常堆栈、请求参数和凭证不得返回前端；允许返回经过白名单与脱敏处理的云商错误码。
...
- 手动/定时健康检测更新快照；
- 技术失败仅记录尝试，不覆盖有效结论；
- 最近技术尝试保存具体结构化错误并投影给列表，但不改变三种主健康状态；
- 账号删除时删除快照。
...
- 云账号表 `version`字段和前端版本参数；并发一致性由后端事务内重读、锁定及配置比较负责；
...
14. 云主体 `(cloudType, accountId)`由数据库唯一约束提供最终并发保护；应用预检测负责友好提示，唯一约束冲突必须转换为统一业务原因。禁用、归档或软删除的同主体记录走恢复，不直接重复新增。
15. 列表在保留旧有效健康结论时，仍能展示最近一次手动或定时检测的具体、脱敏技术失败原因与重新检测操作。
16. 主凭证和扩展凭证均由后端脱敏，通用 `ext`响应不得泄漏 `masterKey / masterSecret`。
17. 探针地域后端配置化；Google只有在 API 启用和真实只读 Vertex AI 权限探针均通过后才返回产品访问 `PASS`。
